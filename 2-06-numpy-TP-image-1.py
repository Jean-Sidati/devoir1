...
# %%
# votre code
im=np.zeros(shape=(91, 91, 3), dtype=np.uint8)+255
#im[:, :, :]=0 noir
#im[:, :, 2]=0 jaune
#print(im[:, 0][0], im[:, 90][90])
im[::  , ::10  , 1]=0
im[:: , ::10 , 0]=0
im[::10  , ::  , 1]=0
im[::10 , :: , 0]=0
...
rect1020=im[261:271, 390:410, :]
rect100200=im[216:316, 300:500, :]
#plt.imshow(rect100200)
...
im[333:533, 600:800, 0]=219
im[333:533, 600:800, 1]=112
im[333:533, 600:800, 2]=147
#plt.imshow(im)

im[333:533, 600:800, 0]=255
im[333:533, 600:800, 1]=255
im[333:533, 600:800, 2]=255
im[333::20, 600::, 1]=0
im[333::20, 600::, 2]=0
#plt.imshow(im)

der=im[513:533, 780:800, :]
#plt.imshow(der)
...
# votre code
plt.imread("les-mines.jpg")
im=np.empty(shape=(533, 800, 4),dtype="uint8")

im[:, :, 0]=plt.imread("les-mines.jpg")[:, :, 0]
im[:, :, 1]=plt.imread("les-mines.jpg")[:, :, 1]
im[:, :, 2]=plt.imread("les-mines.jpg")[:, :, 2]
im[:, :, 3]=128
#plt.imshow(im)
...
# votre code
plt.imread("les-mines.jpg")
im=np.ndarray.astype(plt.imread("les-mines.jpg"),"float")/255

im2=np.empty(shape=(533, 800, 1),dtype="float")
im2[:, :, 0]=im[:, :, 0]*0.299+im[:, :, 1]*0.587+im[:, :, 2]*0.114
#plt.imshow(im2, cmap='gray')

im3=np.empty(shape=(533, 800, 1),dtype="float")
im3[:, :, 0]=(im[:, :, 0]+im[:, :, 1]+im[:, :, 2])/3
...
im4=np.empty(shape=(533,800,1),dtype="float")
im4[:, :, 0]=255*im2[:, :, 0]
im4.astype('uint8')
#plt.imshow(im4,cmap='gray')

#im2 image par correction, im3 image par moyenne, im4 im2 en uint8
...
im1=np.empty(shape=(533, 800, 1), dtype="float")
im1[:, :, 0]=(im[:, :, 0]+im[:, :, 1]+im[:, :, 2])/3

im2=np.empty(shape=(533, 800, 1), dtype="float")
im2[:, :, 0]=im[:, :, 0]*0.299+im[:, :, 1]*0.587+im[:, :, 2]*0.114

im3=np.empty(shape=(533, 800, 3), dtype="float")
im3[:, :, 0]=im[:, :, 0]**(1/2)
im3[:, :, 1]=im[:, :, 1]**(1/2)
im3[:, :, 2]=im[:, :, 2]**(1/2)
